...
$slug = $_GET['slug'] ?? '';
if (!$slug) {
  header('Location: /industries', true, 302);
  exit;
}

$stmt = db()->prepare("SELECT * FROM industries WHERE slug = ? AND status = 1 LIMIT 1");
$stmt->execute([$slug]);
$industry = $stmt->fetch();
...
$metaTitle       = $industry['hero_title']     ?: ($industry['name'] . ' — ' . $siteName);
$metaDescription = $industry['hero_subtitle']  ?: substr(strip_tags($industry['short_desc'] ?? $industry['description'] ?? ''), 0, 160);
$currentUrl      = SITE_URL . '/industries/' . $industry['slug'];
$lang            = currentLang();
...
<main>
  <section class="hero" style="min-height:auto;padding-top:var(--space-40);padding-bottom:var(--space-16);">
    ...
    <div class="container">
      <div class="hero__content" style="max-width:700px;">
        <div class="hero__badge"><span class="hero__badge-dot"></span> <?= ht('Industry') ?></div>
        <h1><?= ht($industry['hero_title'] ?: $industry['name']) ?></h1>
        <p class="hero__description"><?= ht($industry['hero_subtitle'] ?: '') ?></p>
      </div>
    </div>
  </section>

  <?php if (!empty($industry['description']) || !empty($industry['body'])): ?>
  <section class="section">
    <div class="container-sm">
      <div class="reveal">
        <span class="section-label"><?= ht('Overview') ?></span>
        <?php if (!empty($industry['body'])): ?>
          <div style="line-height:1.75;font-size:1.0625rem;color:var(--color-mid);"><?= $industry['body'] ?></div>
        <?php else: ?>
          <p style="font-size:1.125rem;line-height:1.75;"><?= ht($industry['description']) ?></p>
        <?php endif; ?>
      </div>
    </div>
  </section>
  ...
  <section class="section section-surface">
    <div class="container">
      <div class="section-header reveal">
        ...
        <h2 class="section-title"><?= ht('The INTSOLCOM Advantage') ?></h2>
      </div>
      <div class="grid-3">
  ...
  <section class="section">
    <div class="container">
      <div class="section-header reveal">
        <span class="section-label section-label--blue"><?= ht('Solutions') ?></span>
        <h2 class="section-title"><?= ht('How We Serve ' . $industry['name']) ?></h2>
      </div>
      <div class="grid-3">
  ...
  <section class="section section-surface">
    <div class="container">
      <div class="section-header reveal">
        ...
        <h2 class="section-title"><?= ht('Business Units Serving ' . $industry['name']) ?></h2>
      </div>
      <div class="grid-2">
        <?php foreach ($relatedUnits as $idx => $ru):
          $buIcons = ['building'=>'🏢','users'=>'👥','brain'=>'🧠','tags'=>'🏷️'];
          $ruIcon = $buIcons[$ru['icon']] ?? '🏢';
        ?>
        <a href="/business-units/<?= h($ru['slug']) ?>" class="card card-hover reveal" style="text-decoration:none;color:inherit;transition-delay:<?= $idx * 0.05 ?>s;">
          <div style="display:flex;align-items:flex-start;gap:var(--space-4);">
            <div class="card__icon" style="flex-shrink:0;"><?= h($ruIcon) ?></div>
            <div>
              <h3><?= ht($ru['name']) ?></h3>
              <p style="font-size:0.9375rem;"><?= ht(substr(strip_tags($ru['description']), 0, 120)) ?>...</p>
            </div>
          </div>
        </a>
        <?php endforeach; ?>
  ...
  <section class="section">
    <div class="container">
      <div class="section-header reveal">
        <span class="section-label section-label--purple"><?= ht('Technology') ?></span>
        <h2 class="section-title"><?= ht('Products for ' . $industry['name']) ?></h2>
      </div>
      <div class="grid-3">
        <?php foreach ($relatedProds as $idx => $rp):
          $prodIcons = ['users'=>'👥','brain'=>'🧠','tags'=>'🏷️','chart'=>'📊'];
          $rpIcon = $prodIcons[$rp['icon']] ?? '⚙️';
        ?>
        <a href="/technology/<?= h($rp['slug']) ?>" class="product-card reveal" style="text-decoration:none;color:inherit;transition-delay:<?= $idx * 0.05 ?>s;">
          <div class="product-card__gradient"></div>
          <div class="product-card__header">
            <div class="card__icon" style="font-size:1.5rem;"><?= h($rpIcon) ?></div>
            <h3><?= ht($rp['name']) ?></h3>
          </div>
          <div class="product-card__body">
            <p><?= ht($rp['short_desc'] ?? '') ?></p>
          </div>
        </a>
        <?php endforeach; ?>
      </div>
  ...
  <section class="cta-section">
    ...
    <div class="container">
      <h2><?= ht('Transform your ' . $industry['name'] . ' operations') ?></h2>
      <p><?= ht('Let\'s discuss how INTSOLCOM technology and expertise can drive results in your industry.') ?></p>
      <div class="cta-section__actions">
        <a href="/contact" class="btn btn-accent btn-lg"><?= ht('Contact us') ?></a>
        <a href="/industries" class="btn btn-outline-white btn-lg"><?= ht('All Industries') ?></a>
      </div>
    </div>
  </section>
</main>
