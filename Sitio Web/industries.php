...
$industries = db()->query("SELECT * FROM industries WHERE status = 1 ORDER BY order_num ASC")->fetchAll();

$siteName   = setting('site_name', 'INTSOLCOM');
$logoText   = setting('logo_text', 'INTSOL');
$logoAccent = setting('logo_accent', 'COM');

$metaTitle       = $page['meta_title']       ?? 'Industries — INTSOLCOM';
$metaDescription = $page['meta_desc']         ?? 'Enterprise solutions across every sector. Healthcare, Technology, Financial Services, Logistics, and more — powered by INTSOLCOM technology.';
$currentUrl      = SITE_URL . '/industries';
$lang            = currentLang();

$indIcons = [
  'healthcare' => '🏥', 'technology' => '💻', 'finance' => '🏦', 'ai' => '🤖',
  'retail' => '🛒', 'logistics' => '🚚', 'realestate' => '🏠', 'legal' => '⚖️',
  'manufacturing' => '🏭', 'hospitality' => '🏨', 'energy' => '⚡', 'education' => '🎓',
  'government' => '🏛️', 'agriculture' => '🌾', 'sports' => '⚽', 'consulting' => '📋',
  'ecommerce' => '🛍️',
];
?>
<!DOCTYPE html>
...
<main>
  <section class="hero" style="min-height:auto;padding-top:var(--space-40);padding-bottom:var(--space-16);">
    ...
    <div class="container">
      <div class="hero__content" style="max-width:700px;">
        ...
        <h1><?= ht('Industries') ?></h1>
        <p class="hero__description"><?= ht('Enterprise solutions across every sector.') ?></p>
      </div>
    </div>
  </section>
    ...
    <div class="container">
      <?php if (empty($industries)): ?>
        <div class="text-center" style="padding:var(--space-16) 0;">
          <p style="color:var(--color-light);font-size:1.125rem;"><?= ht('No industries found.') ?></p>
        </div>
      <?php else: ?>
        <div class="grid-auto">
          <?php foreach ($industries as $idx => $ind):
            $iconKey = strtolower(str_replace([' ','&','/'],'', $ind['icon'] ?? $ind['name'] ?? ''));
            $iconChar = $indIcons[$iconKey] ?? '🏢';
          ?>
          <a href="/industries/<?= h($ind['slug']) ?>" class="industry-card reveal" style="text-decoration:none;transition-delay:<?= $idx * 0.03 ?>s;">
            <div class="industry-card__icon"><?= h($ind['icon'] ?: $iconChar) ?></div>
            <h3><?= ht($ind['name']) ?></h3>
            <p><?= ht($ind['short_desc'] ?? '') ?></p>
          </a>
          <?php endforeach; ?>
        </div>
      <?php endif; ?>
  ...
  <section class="cta-section" style="margin-top:var(--space-16);">
    ...
    <div class="container">
      <h2><?= ht('Don\'t see your industry?') ?></h2>
      <p><?= ht('Our technology platforms are industry-agnostic. Contact us to discuss how we can adapt to your sector\'s specific needs.') ?></p>
